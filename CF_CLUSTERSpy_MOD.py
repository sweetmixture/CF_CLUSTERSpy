...
class cf_cluster_mod:
	...
	def load_xyz(self):
		...
		try:
			with open(self.xyz_file_1,"r") as f1:
				...
				rws       = f1.readline()
				self.c1_config = [[0 for i in range(4)] for j in range(self.c1_n+2)]	# HERE +2 IS FOR DUMMY ATOM WS: adding 'ghost atom(s)'
                                                                                                        # getting rid of degeneracy in the principal moment
		...
		for i in range(self.c1_n):
			...
			rms_org = rms_org + dx**2. + dy**2. + dz**2.
		self.rms_config_org = math.sqrt(rms_org)/3./float(self.c1_n)
	...
	def eig_support_match(self, config1, vec1, config2, vec2, noa):
		...
		key_idx = list(rms.keys())
                ...
                # Matching Table : go through octants and figure out the best matching rotation
	...
	def show_info(self,*args):
		...
		if 'com' in args:
			print("#"*80)
			print("CENTRE OF MASS CLUSTER 1 : ",end="")
